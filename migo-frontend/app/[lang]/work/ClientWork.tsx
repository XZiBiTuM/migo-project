...
import {
  Briefcase, MessageCircle, MapPin, CheckCircle2, ShieldCheck, XCircle, Send, FileText, Home, ArrowRight, Clock, Search
} from 'lucide-react';
...
export default function ClientWork({ initialJobs = [], lang }: { initialJobs?: VacancyItem[], lang: string }) {
  const t = getT(lang);
    ...
    <main className="pb-24 bg-[#F8FAFC] selection:bg-[#B8D430]/30 min-h-screen">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className={`flex flex-col items-center lg:items-start text-center lg:text-left`}>
              <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/60 backdrop-blur-md border border-white/80 shadow-sm mb-8">
                ...
                <p className="text-xs md:text-sm font-black text-[#1E58B1] uppercase tracking-[0.25em]">{t('work.hero.badge', 'Проверенные вакансии')}</p>
              </div>

              <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-[#163A5C] leading-[0.95] mb-8 tracking-tighter">
                {t('work.hero.title_1', 'Работа в России')}<br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#1E58B1] to-[#2196D3]">{t('work.hero.title_2', 'через Telegram')}</span>
              </h1>

              <p className="text-lg md:text-2xl text-gray-700/80 mb-12 max-w-xl leading-relaxed font-medium">
                {t('work.hero.subtitle', 'Без анкет и регистраций. Найдите работу за 2 минуты через удобного бота под контролем менеджера.')}
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                ...
                >
                  <Briefcase size={28} /> {t('work.hero.btn', 'Найти работу в Telegram')}
                </Link>
              </div>
            </div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-5">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-black text-[#163A5C] mb-6">{t('work.steps.title', 'Как это работает')}</h2>
          <p className="text-gray-700 text-lg max-w-2xl mx-auto">{t('work.steps.subtitle', 'Мы сделали процесс поиска работы максимально прозрачным и безопасным.')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
          ...
          {[
            { num: '01', title: t('work.steps.step1_title', 'Нажмите кнопку'), desc: t('work.steps.step1_desc', '«Найти работу» на этом сайте'), icon: <MessageCircle /> },
            { num: '02', title: t('work.steps.step2_title', 'Откроется бот'), desc: t('work.steps.step2_desc', 'Ваш помощник в Telegram'), icon: <Search /> },
            { num: '03', title: t('work.steps.step3_title', 'Выберите город'), desc: t('work.steps.step3_desc', 'И желаемую профессию'), icon: <MapPin /> },
            { num: '04', title: t('work.steps.step4_title', 'Получите вакансии'), desc: t('work.steps.step4_desc', 'И напишите менеджеру напрямую'), icon: <CheckCircle2 /> },
          ].map((step, idx) => (
        ...
        <div className="max-w-7xl mx-auto px-5">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div className="max-w-2xl">
              <h2 className="text-3xl md:text-5xl font-black text-[#163A5C] mb-4">{t('work.advantages.title', 'Преимущества MIGO')}</h2>
              <p className="text-gray-700 text-lg">{t('work.advantages.subtitle', 'Мы не просто ищем работу, мы обеспечиваем безопасность вашего трудоустройства.')}</p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            <AdvantageCard icon={<ShieldCheck />} title={t('work.advantages.adv1_title', 'Проверка работодателей')} desc={t('work.advantages.adv1_desc', 'Работаем только с проверенными компаниями')} />
            <AdvantageCard icon={<Home />} title={t('work.advantages.adv2_title', 'Помощь с жильём')} desc={t('work.advantages.adv2_desc', 'Подберем общежитие рядом с работой')} />
            <AdvantageCard icon={<FileText />} title={t('work.advantages.adv3_title', 'Консультация по документам')} desc={t('work.advantages.adv3_desc', 'Поможем со списком для патента/регистрации')} />
            <AdvantageCard icon={<MessageCircle />} title={t('work.advantages.adv4_title', 'Поддержка 24/7')} desc={t('work.advantages.adv4_desc', 'Менеджер всегда на связи в Telegram')} />
            <AdvantageCard icon={<Briefcase />} title={t('work.advantages.adv5_title', 'Оплата без обмана')} desc={t('work.advantages.adv5_desc', 'Контролируем прозрачность выплат')} />
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-[#F8FAFC] border-y border-gray-100 relative">
        <div className="max-w-7xl mx-auto px-5">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div className="max-w-2xl">
              <h2 className="text-3xl md:text-5xl font-black text-[#163A5C] mb-4">{t('work.vacancies.title', 'Популярные вакансии')}</h2>
              <p className="text-gray-700 text-lg">{t('work.vacancies.subtitle', 'Примеры актуальных предложений. Напишите в Telegram, чтобы получить полный список и подобрать работу индивидуально.')}</p>
            </div>
            <Link
            ...
            >
              {t('work.vacancies.all_in_tg', 'Все вакансии в Telegram')} <ArrowRight size={18} />
            </Link>
          </div>
          ...
          {initialJobs.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-3xl border border-dashed border-gray-200">
              <p className="text-gray-700 font-medium">{t('work.vacancies.no_vacancies', 'Пока нет доступных вакансий. Загляните позже или напишите нам в Telegram для индивидуального подбора.')}</p>
            </div>
          )}
          ...
          <div className="mt-12 text-center">
            ...
            >
              <Briefcase size={20} /> {t('work.vacancies.other_options', 'Подобрать другие варианты')}
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-5">
        <h2 className="text-3xl md:text-5xl font-black text-center text-[#163A5C] mb-16">{t('work.safety.title', 'Безопасность прежде всего')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          <div className="bg-red-50/30 border border-red-100 p-10 rounded-[40px] group transition-all bg-white shadow-2xl relative overflow-hidden">
            <div className="absolute top-4 right-6 px-3 py-1 bg-red-100 text-red-600 text-[10px] font-black uppercase tracking-widest rounded-full">
              {t('work.safety.risks_label', 'Риски')}
            </div>
            <h3 className="text-2xl font-black text-red-600/60 mb-8 flex items-center gap-3">
              <XCircle className="text-red-400" /> {t('work.safety.risks_title', 'Случайные чаты')}
            </h3>
            <ul className="space-y-6">
              <li className="flex gap-4 items-start">
                ...
                <p className="text-gray-700 font-medium leading-relaxed">{t('work.safety.risk1', 'Нет никакой проверки работодателя и условий труда')}</p>
              </li>
              <li className="flex gap-4 items-start">
                <div className="w-6 h-6 rounded-full bg-red-100 flex items-center justify-center shrink-0 mt-1"><XCircle size={14} className="text-red-500" /></div>
                <p className="text-gray-700 font-medium leading-relaxed">{t('work.safety.risk2', 'Высокий риск обмана с зарплатой и графиком')}</p>
              </li>
              <li className="flex gap-4 items-start">
                <div className="w-6 h-6 rounded-full bg-red-100 flex items-center justify-center shrink-0 mt-1"><XCircle size={14} className="text-red-500" /></div>
                <p className="text-gray-700 font-medium leading-relaxed">{t('work.safety.risk3', 'Никто не несет ответственности за вашу безопасность')}</p>
              </li>
            </ul>
          </div>
          ...
          <div className="bg-white border border-[#2196D3]/20 p-10 rounded-[40px] shadow-2xl relative overflow-hidden group">
            ...
            <ul className="space-y-6">
              <li className="flex gap-4 items-start">
                ...
                <p className="text-[#163A5C] font-bold leading-relaxed">{t('work.safety.migo1', 'Только проверенные вакансии с официальным оформлением')}</p>
              </li>
              <li className="flex gap-4 items-start">
                <div className="w-6 h-6 rounded-full bg-[#B8D430]/20 flex items-center justify-center shrink-0 mt-1"><CheckCircle2 size={14} className="text-[#1E58B1]" /></div>
                <p className="text-[#163A5C] font-bold leading-relaxed">{t('work.safety.migo2', 'Помощь в решении бытовых вопросов и жилья')}</p>
              </li>
              <li className="flex gap-4 items-start">
                <div className="w-6 h-6 rounded-full bg-[#B8D430]/20 flex items-center justify-center shrink-0 mt-1"><CheckCircle2 size={14} className="text-[#1E58B1]" /></div>
                <p className="text-[#163A5C] font-bold leading-relaxed">{t('work.safety.migo3', 'Полное сопровождение менеджером до выхода на работу')}</p>
              </li>
            </ul>
          </div>
        </div>
      ...
      <section className="py-20 max-w-7xl mx-auto px-5 text-center">
        <p className="text-[#163A5C]/40 font-black uppercase tracking-[0.2em] text-sm mb-10">{t('work.countries.title', 'Работаем для граждан из')}</p>
        <div className="flex flex-wrap justify-center gap-4">
          {[
            ...
            <div key={country.code} className="px-8 py-4 bg-white border border-gray-100 rounded-3xl font-black text-lg text-[#163A5C] shadow-sm transform hover:scale-105 transition-all flex items-center gap-3">
              ...
              {t(`work.countries.${country.code}`, country.name)}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-5">
        <div className="bg-gradient-to-br from-[#1E58B1] via-[#1A4B75] to-[#2196D3] rounded-[48px] p-8 md:p-20 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            <h2 className="text-4xl md:text-6xl font-black text-white mb-8">{t('work.cta.title', 'Готовы начать работать?')}</h2>
            <p className="text-white/80 text-xl md:text-2xl mb-12 leading-relaxed font-medium">
              {t('work.cta.subtitle', 'Не нужно ничего искать. Просто напишите нам в Telegram, и мы предложим лучшие вакансии сегодня.')}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href={getBotUrl({ start: 'work', source: 'site_work_page', medium: 'work' })} target="_blank" className="bg-[#B8D430] hover:bg-[#A7C220] text-[#163A5C] py-5 px-12 rounded-[24px] font-black text-xl flex items-center justify-center gap-3 transition-all hover:scale-105 shadow-2xl">
                <MessageCircle size={28} /> {t('work.cta.btn_tg', 'Написать в Telegram')}
              </Link>
              <Link href={getBotUrl({ start: 'law', source: 'site_work_page', medium: 'work' })} target="_blank" className="bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 text-white py-5 px-12 rounded-[24px] font-black text-xl flex items-center justify-center gap-3 transition-all hover:scale-105">
                {t('work.cta.btn_legal', 'Юридическая помощь')}
              </Link>
            </div>
          </div>
        </div>
...
function JobCard({ job, lang }: { job: VacancyItem, lang: string }) {
  const t = getT(lang);
  const tags = [];
  ...
  return (
    <div className="bg-white rounded-[48px] p-8 md:p-10 border border-gray-100 hover:border-[#B8D430] hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] transition-all duration-500 group flex flex-col justify-between h-full shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-8 border-t border-gray-50 mt-4">
        ...
        >
          <Send size={20} /> {t('work.vacancies.apply_tg', 'Откликнуться в Telegram')}
        </Link>
      </div>
    </div>
  );
