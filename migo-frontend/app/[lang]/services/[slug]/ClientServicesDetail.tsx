...
export default function ClientServicesDetail({ service, lang }: { service: any, lang: string }) {
  const t = getT(lang);
    ...
    <main className="pt-28 md:pt-32 pb-20 min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 mb-12">
        <Link href={`/${lang}/services`} className="inline-flex items-center gap-2 text-gray-700 hover:text-[#2196D3] font-medium transition-colors mb-8 cursor-pointer">
           <ArrowLeft size={20} /> {t('service_detail.back_btn', 'Ко всем услугам')}
        </Link>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex-1">
            <span className="inline-block px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider text-[#2196D3] bg-[#2196D3]/10 mb-4">
              {service.service_type === 'docs' ? t('services.categories.docs', 'Документы') : service.service_type === 'finance' ? t('services.categories.finance', 'Финансы') : t('services.categories.other', 'Сервис')}
            </span>
          ...
          <div className="w-full md:w-auto shrink-0">
            ...
            >
              <Send className="w-6 h-6" /> {t('service_detail.cta_btn', 'Оформить в Telegram')}
            </Link>
          </div>
        </div>
      </div>
          ...
          <div className="md:col-span-2 space-y-12">
            ...
            <section>
              <h2 className="text-2xl font-bold text-[#163A5C] mb-6 flex items-center gap-3">
                <ShieldCheck className="text-[#2196D3]" /> {t('service_detail.desc_title', 'Описание услуги')}
              </h2>
            ...
            {getLocalized('documents_required') && (
              <section className="bg-gray-50 rounded-3xl p-8 border border-gray-100">
                <h2 className="text-2xl font-bold text-[#163A5C] mb-6 flex items-center gap-3">
                  <FileText className="text-[#2196D3]" /> {t('service_detail.docs_title', 'Необходимые документы')}
                </h2>
            ...
            <section className="bg-[#1E58B1] text-white rounded-[2rem] p-10 relative overflow-hidden">
              ...
              <h3 className="text-2xl font-bold mb-4 relative z-10">{t('service_detail.cta_questions_title', 'Остались вопросы?')}</h3>
              <p className="text-white/90 mb-8 relative z-10">{t('service_detail.cta_questions_desc', 'Наши специалисты помогут собрать правильный пакет документов и ответят на все юридические вопросы.')}</p>
              <Link href={getBotUrl({ start: 'consult', source: 'service_detail', medium: 'web' })} target="_blank" className="inline-flex items-center gap-2 bg-[#00BCD4] text-white px-8 py-4 rounded-xl font-bold hover:scale-105 transition-transform cursor-pointer">
                <MessageCircle size={20} /> {t('service_detail.cta_consult_btn', 'Написать консультанту')}
              </Link>
            </section>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
              <div className="flex items-center gap-3 mb-4 text-[#2196D3]">
                <Clock size={24} />
                <span className="font-bold uppercase tracking-wider text-xs">{t('service_detail.time_label', 'Сроки')}</span>
              </div>
              <p className="text-xl font-bold text-[#163A5C]">{getLocalized('processing_time') || t('service_detail.time_placeholder', 'Уточняйте в боте')}</p>
            </div>

            <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
              <div className="flex items-center gap-3 mb-4 text-[#B8D430]">
                <CreditCard size={24} />
                <span className="font-bold uppercase tracking-wider text-xs">{t('service_detail.price_label', 'Стоимость')}</span>
              </div>
            ...
            {service.is_partner_service && (
              <div className="bg-orange-50 rounded-2xl p-6 border border-orange-100">
                <p className="text-sm text-orange-800 font-medium">
                  ⚠️ {t('service_detail.partner_disclaimer', 'Услуга предоставляется нашими проверенными партнёрами. MIGO осуществляет сопровождение и контроль качества.')}
                </p>
              </div>
            )}
          </div>
